 ...
 */
class DidExchangeV1Protocol(mex: MessageExchange): Protocol<DidExchangeV1Protocol>(mex) {
    override val log = KotlinLogging.logger {}

    override val protocolUri = DIDEXCHANGE_PROTOCOL_V1.uri

    companion object {
        val DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1 = "${DIDEXCHANGE_PROTOCOL_V1.uri}/request"
        val DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1 = "${DIDEXCHANGE_PROTOCOL_V1.uri}/response"
        val DIDEXCHANGE_MESSAGE_TYPE_COMPLETE_V1 = "${DIDEXCHANGE_PROTOCOL_V1.uri}/complete"
    ...
    override fun invokeMethod(to: Wallet, messageType: String): Boolean {
        when (messageType) {
            DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1 -> receiveDidExchangeRequest(to)
            DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1 -> receiveDidExchangeResponse(to)
            DIDEXCHANGE_MESSAGE_TYPE_COMPLETE_V1 -> receiveDidExchangeComplete(to)
            else -> throw IllegalStateException("Unsupported message type: $messageType")
        }
        return true
    }

    fun connect(requester: Wallet, timeout: Int = 10, unit: TimeUnit = TimeUnit.SECONDS): DidExchangeV1Protocol {
        ...
        mex.withProtocol(TRUST_PING_PROTOCOL_V1)
            .sendTrustPing()
    ...
    fun sendDidExchangeRequest(requester: Wallet): DidExchangeV1Protocol {
        ...
        mex.placeEndpointMessageFuture(DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1)
        ...
        val didexRequest = """
                {
                    "@type": "$DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1",
                    "@id": "$didexReqId",
                    "~thread": {
                        "thid": "$didexReqId",
                        "pthid": "${invitation.id}"
    ...
    fun awaitDidExchangeResponse(timeout: Int = 10, unit: TimeUnit = TimeUnit.SECONDS): DidExchangeV1Protocol {
        mex.awaitEndpointMessage(DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1, timeout, unit)
        return this
    }

    fun sendDidExchangeComplete(): DidExchangeV1Protocol {

        val requester = mex.getAttachment(WALLET_ATTACHMENT_KEY) as Wallet
        check(requester.agentType == AgentType.NESSUS) { "Requester must be Nessus" }

        val didexResponse = mex.last
        mex.checkLastMessageType(DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1)
        ...
        val didexComplete = """
                {
                    "@type": "$DIDEXCHANGE_MESSAGE_TYPE_COMPLETE_V1",
                    "@id": "${UUID.randomUUID()}",
                    "~thread": {
                        "thid": "$didexThid",
                        "pthid": "${invitation.id}"
    ...
    private fun receiveDidExchangeRequest(responder: Wallet): Wallet {
        ...
        didexRequest.checkMessageType(DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1)
        ...
        mex.placeEndpointMessageFuture(DIDEXCHANGE_MESSAGE_TYPE_COMPLETE_V1)

        if (mex.hasEndpointMessageFuture(DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1))
            mex.completeEndpointMessageFuture(DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1, mex.last)
    ...
    private fun sendDidExchangeResponse(responder: Wallet) {
        ...
        mex.checkLastMessageType(DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1)
        ...
        val didexResponse = """
        {
            "@type": "$DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1",
            "@id": "${UUID.randomUUID()}",
            "~thread": {
                "thid": "$didexThid",
                "pthid": "${invitation.id}"
    ...
    private fun receiveDidExchangeResponse(requester: Wallet): Wallet {
        ...
        val didexResponse = mex.last
        didexResponse.checkMessageType(DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1)
        ...
        mex.completeEndpointMessageFuture(DIDEXCHANGE_MESSAGE_TYPE_RESPONSE_V1, mex.last)
    ...
    private fun receiveDidExchangeComplete(responder: Wallet): Wallet {
        ...
        mex.placeEndpointMessageFuture(TRUST_PING_MESSAGE_TYPE_PING_V1)
        mex.completeEndpointMessageFuture(DIDEXCHANGE_MESSAGE_TYPE_COMPLETE_V1, mex.last)
    ...
    private fun awaitDidExchangeRequest(): EndpointMessage {
        return mex.awaitEndpointMessage(DIDEXCHANGE_MESSAGE_TYPE_REQUEST_V1)
    }

    private fun awaitDidExchangeComplete(): EndpointMessage {
        return mex.awaitEndpointMessage(DIDEXCHANGE_MESSAGE_TYPE_COMPLETE_V1)
    }

    private fun awaitTrustPing(timeout: Int, unit: TimeUnit): EndpointMessage {
        return mex.awaitEndpointMessage(TRUST_PING_MESSAGE_TYPE_PING_V1, timeout, unit)
    }
}
