...
class Lab1DidExchangeTest : AbstractIntegrationTest() {
    ...
    fun didExchange_FaberAcapy_AliceAcapy() {
        ...
        inviter.openWebSocket {
            log.info { "WebSocket ${inviter.alias}: ${it.topic}" }
            log.info { it.payload.sortedJson().prettyPrint() }
        }

        invitee.openWebSocket {
            log.info { "WebSocket ${invitee.alias}: ${it.topic}" }
            log.info { it.payload.sortedJson().prettyPrint() }
        }
        ...
        try {
            ...
            if (!inviterAutoAccept) {
                val acceptInvitationFilter = DidExchangeAcceptInvitationFilter()
                acceptInvitationFilter.myEndpoint = inviter.endpointUrl
                acceptInvitationFilter.myLabel = "Accept Faber/Alice"
